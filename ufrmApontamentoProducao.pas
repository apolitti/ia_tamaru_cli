...
begin
  ...
  try
    ...
    DM.p_apontamento_lst(Cl_Apo,StrToInt(sAPO_IN_SEQUENCIA));

    //impressão
    if (Cl_Ord.FieldByName('etiq_ch_momentoimpauto').AsString = 'D')
    then
    begin
...
begin
  ...
  if Cl_Ord.FieldByName('etiq_in_id').AsInteger > 0 then
  begin

    DM.p_impressora_ma_lst(Cl_Imp, Cl_Ord.FieldByName('FIL_IN_CODIGO').AsInteger, Cl_Cmq.FieldByName('cmaq_in_codigo').AsInteger);

    if Cl_Imp.IsEmpty = True then
    begin
      raise Exception.Create('Impressora não definida para máquina!');
    end;
